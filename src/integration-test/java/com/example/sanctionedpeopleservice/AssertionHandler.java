...
public class AssertionHandler {
  ...
  public static void assertExistingPersonError(Long personId, ApiError error) {
    ...
    assertEquals(1, error.getErrors().size());
    assertEquals("Person with id: ".concat(personId.toString()).concat(" already exist in sanctioned people list!"),
                 error.getErrors().get(0).getMessage());
  }
}